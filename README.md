...
- 自动任务周期在“测速”弹窗里按小时设置（1 到 72，默认 24）。任何一轮任务结束都重新起算，进度面板显示“下次自动任务：X 小时 Y 分钟后”。
- “跳过 N 小时内已测过的节点”（默认 12）只影响测速候选：这段时间内测过速的节点本轮不再测，0 表示全部重测。
- 进度面板显示当前阶段、检测与测速进度、当前节点与本轮最快节点，可随时“停止任务”，已得结果保留。空闲时显示上一轮结果与下次自动任务时间。
- 检测与测速都使用独立的测试隧道，期间当前连接保持不变，连接卡片与顶部状态仍显示“已连接”；只有真正切换节点时才显示“正在连接”。隧道断开且出口接管开启时显示“出口已阻断”。
