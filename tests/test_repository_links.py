...
class RepositoryLinkTests(unittest.TestCase):
    def test_readme_uses_owner_install_commands_without_ads(self):
        ...
        self.assertIn(f"bash <(curl -fL {owner}install.sh)", readme)
        self.assertIn(f"bash <(curl -fL {owner}jkw.sh)", readme)
        for removed in (
            "baoweise-bot",
            "BandwagonHost",
            "RackNerd",
    ...
    def test_installer_defaults_to_owner_repository(self):
        ...
        self.assertIn('subprocess.run(["/usr/local/sbin/jkw", "--撤销"], check=False)', installer)
